...
thius si because more weight to the boxes that are opened, thus 
(find the probabilities) if many blue boxes are opened, prob of red being major colour is lower now than for the unif prior. Then, the exp loss of choosing blue as major colour is lower. Thus, taht exp loss migh be lower earlier than 

alpha lower makes sense because the model giver more weight to the boxes that are opened already. if many blue boxes opened, then the prob that the major colour is blue is higher. that means that if particioapnt chooses early, it is more compatible(?) with the model with the new prior for $\Theta$. 
...
we see that the alpha values tend to be lower than for teh unfirm prior, but the eta vlaues tend to be higher. 
