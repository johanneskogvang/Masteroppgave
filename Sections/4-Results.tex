...
Figure \ref{fig:sensitivity_unlim_cis_eta} displays the confidence intervals for $\eta$ for each participant. We also have here that the orange dots are the CIs in the case with the new prior and the green lines are the CIs we found in the case with the uniform prior. These intervals differ more than the intervals for $\alpha$. 
Some of the new intervals are shorter and others are longer than the green lines, withput there being any clear pattern of which ones. They might be slightly shifted to the right. At least for the intervals that are longer than the original intervals. There it is mostly the upper limit that is higher than the original, and the lower limit is often close to the lower limit of the green line. 
However, these higher upper limits might be because of the flat log likelihood function. 

%The high values of $\eta$ are therefore not reliable, such that if the limit is high, for example higher than 1000, which is the threshold value we defined, we could say that the upper limit is infinity. 

they are both longer and shorter. 
are tehy shifted to one side? maybe a bit more to the right. at least if longer intervlas, it is mostly the upper limit that is shifted. but that might be because the flat log likel func, thus high values are not necesarilly the 'right' vlaues, but they are 'infinity'. 
\begin{figure}
    \centering
