...
%\section{Theory}
In this section we will go through some of the statistical theory used in this report. That includes the theorem of total probability, Bayes' theorem, the beta and gamma functions, Bayesian modelling, loss functions, the softmax function, maximum likelihood estimation and bootstrapping.  
...
Let $A_i$ be a partition of the sample space, $S$. Then, there are $k$ non-overlapping parts, such that $A_i \cap A_j = \emptyset$, $\forall \:
...
As an example, consider that the $\delta_i$'s have probability mass function as in \eqref{softmax_in_theory}. The parameters that we want to find estimates for are then $\theta$ and $\eta$. If we have $n$ samples of $\Delta$, denoted $\delta_i$, where $i \in \{1,2,...,n\}$,
...
Consider a sample, $(\delta_1,\delta_2,...,\delta_n)$, where the samples are identically and independently distributed from an unknown distribution, $F$. We can then use this sample to estimate this distribution, thus, find $\hat{F}$. To get some ideas about the properties of $F$, we can find the properties of $\hat{F}$. It is often hard to do this analytically, then we can do it with simulations instead. This is where bootstrapping is useful. Bootstrapping is a way to find those samples, either from the original sample, $(\delta_1,\delta_2,...,\delta_n)$, or from the estimated distribution, $\hat{F}$. We can then use those samples to for example find standard error, bias, variance, or perhaps the most common, confidence intervals \citep{bootstrap}.
...
When we have $B$ bootstrap samples, there are multiple methods for finding confidence intervals. 
